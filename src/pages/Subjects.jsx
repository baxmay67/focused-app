...
export default function Subjects() {
  ...
  const load = async () => {
    try {
      const res = await axios.get(`http://fwh.is/subjects.php?user_id=${user.id}`);
  ...
  const add = async (e) => {
    ...
    setSaving(true);
    try {
      const res = await axios.post("http://fwh.is/subjects.php", {
        user_id: user.id,
        name: form.name,
  ...
  const remove = async (id) => {
    try {
      await axios.delete("http://fwh.is/subjects.php", {
        data: { id },
      });
